...
| `fs`                | read/write/stat/copy/rename/remove/truncate, `FileHandle`, line iteration, watchers, scope and deny patterns (recursive `remove`, `readDir`).                          | All but the watchers (`fs:allow-watch` is desktop-only here).                            |
