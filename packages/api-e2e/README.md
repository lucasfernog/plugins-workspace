...
| `deep-link`         | `getCurrent` on a plain launch, `onOpenUrl` delivery, runtime `register`/`isRegistered`/`unregister` and invalid scheme rejection on Linux and Windows.                | Only that runtime registration is unsupported.                                           |
