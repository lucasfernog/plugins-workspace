...
| `shell`             | `execute`, `spawn` with stdout/stderr/close events, stdin, `kill`, scope enforcement (allow/deny, `env`/`cwd`).                                                        | Scope only on iOS, which cannot spawn a process at all.                                  |
