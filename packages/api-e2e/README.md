...
| `shell`             | `execute`, `spawn` with stdout/stderr/close events, stdin, `kill` (also with a blocked write), scope enforcement.                                                      | Scope only on iOS, which cannot spawn a process at all.                                  |
