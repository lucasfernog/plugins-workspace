...
| `geolocation`       | Skipped — mobile-only plugin.                                                                                                                                          | `checkPermissions`, `clearWatch`, and that `watchPosition` settles (Android).            |
...
| mobile-only plugins | `barcode-scanner`, `biometric`, `haptics` and `nfc` need hardware or native UI that the driver cannot operate, so only their global API surface is ever asserted here. | Only there.                                                                              |
