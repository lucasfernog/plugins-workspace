...
| `websocket`         | Text/binary echo, ping/pong, handshake headers, size limits, listener removal, server/client close, connection and argument errors, against the fixture server.        | Same (through `adb reverse` on Android).                                                 |
