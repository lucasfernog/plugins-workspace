...
| `websocket`         | Text/binary echo, ping/pong, handshake headers, listener removal, server/client close, URL scope, connection and argument errors, against the fixture server.          | Same (through `adb reverse` on Android).                                                 |
