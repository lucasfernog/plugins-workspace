...
| `barcode-scanner`   | `checkPermissions`, `cancel`, and that every scan settles: cancelled on Android, rejected for the missing camera on the iOS simulator. Nothing is ever scanned. | Only there (mobile-only plugin).                                                         |
...
| mobile-only plugins | `biometric`, `geolocation`, `haptics` and `nfc` need hardware or native UI the driver cannot operate, so only their global API surface is asserted.             | Only there.                                                                              |
