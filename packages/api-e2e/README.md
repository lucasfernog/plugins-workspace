...
| `dialog`            | The `window.alert`/`window.confirm` overrides call `message`. No dialog is opened: they block on native UI the driver cannot operate.                                  | iOS only; Android has no overrides.                                                      |
