...
| `haptics`           | Every command resolves to an `ok` result, `vibrate(0)` included; invalid input resolves to an `error` result instead of rejecting. The feedback is not observable.     | Only there — mobile-only plugin.                                                         |
...
| mobile-only plugins | `barcode-scanner`, `biometric`, `geolocation` and `nfc` need real hardware or native UIs that the driver cannot operate, so only their global API surface is asserted. | Only there.                                                                              |
