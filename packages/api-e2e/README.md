...
| `notification`      | Permission model, the `window.Notification` override, and that sending leaves the caller's options untouched. Display is not observable.                               | Action types, pending/active lists after cancel/remove, channels, listeners — see below. |
