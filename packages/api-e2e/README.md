...
| `biometric`         | `BiometryType` values, `checkStatus` shape and stability, iOS prompt-less `authenticate` rejection. A real authentication is never exercised.                          | Only there — mobile-only plugin.                                                         |
...
| mobile-only plugins | `barcode-scanner`, `geolocation`, `haptics` and `nfc` need hardware or native UI the driver cannot operate, so only their global API surface is asserted by the suite. | Only there.                                                                              |
