...
| `shell`             | `execute`, `spawn` with stdout/stderr/close events, stdin, `kill`, scope enforcement (allow/deny, argument errors).                                                    | Scope only on iOS, which cannot spawn a process at all.                                  |
