...
| `positioner`        | Screen and tray positions (tray rect handed in via `handleIconState`), `moveWindowConstrained`, the missing-tray error, the top-edge flip on Windows (WM dependent).   | Skipped — desktop-only plugin.                                                           |
