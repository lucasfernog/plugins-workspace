...
| `nfc`               | Skipped — mobile-only plugin.                                                                                                                                   | The `record`/`textRecord`/`uriRecord` builders, `isAvailable`, and `scan`/`write` rejecting when NFC is unavailable (skipped on a device with NFC). Scanning and writing need a physical tag. |
...
| mobile-only plugins | `barcode-scanner`, `biometric`, `geolocation` and `haptics` need hardware or native UI the driver cannot operate, so only their global API surface is asserted. | Only there.                                                                                                                                                                                   |
