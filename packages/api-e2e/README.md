...
| `upload`            | `download` and `upload` with progress, methods, headers, error paths, and a failed download keeping the existing file.                                                 | Same (through `adb reverse` on Android).                                                 |
