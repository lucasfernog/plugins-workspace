...
Note that the Plugin will only emit events on macOS, iOS and Android. On Windows and Linux the OS will spawn a new instance of your app with the URL as a CLI argument, and `getCurrent` only reports that launch URL, for the schemes configured in `desktop` (schemes registered at runtime must be read from the arguments yourself). If you want your app to behave on Windows & Linux similar to the other platforms you can use the [single-instance](../single-instance/) plugin with the `deep-link` feature enabled.

### Desktop platforms

- **macOS**: the schemes are written into the app bundle's `Info.plist` by the bundler, so deep links only work with a bundled `.app` (not with `tauri dev`). Runtime registration is unsupported.
- **Windows and Linux**: the installers register the configured schemes. To register them at runtime as well, for example for an AppImage or during development on Windows, call `register_all` (Rust) on those platforms, or `register` (Rust and JavaScript) for individual schemes:
  ...
  On Linux, runtime registration writes `~/.local/share/applications/<binary name>-handler.desktop` and needs the `xdg-mime` and `update-desktop-database` commands (the `xdg-utils` and `desktop-file-utils` packages).

## Permissions

By default only `getCurrent` is allowed (`deep-link:default`). To use `register`, `unregister` and `isRegistered` from JavaScript, add `deep-link:allow-register`, `deep-link:allow-unregister` and `deep-link:allow-is-registered` to your capability. See [the permission reference](permissions/autogenerated/reference.md).
