...
The `nfc:default` permission set allows `isAvailable()` and `scan()`. Writing to tags is not part of it, so an app that calls `write()` must also grant `nfc:allow-write`, for instance in `src-tauri/capabilities/mobile.json`:
...
```json
{
  ...
  "windows": ["main"],
  "permissions": ["nfc:default", "nfc:allow-write"]
}
```

See [permissions/autogenerated/reference.md](permissions/autogenerated/reference.md) for every permission.
